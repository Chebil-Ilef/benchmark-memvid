...
DEFAULT_QUERIES_FILE = Path("./test_queries/energie_certificates_queries.txt")
DEFAULT_DATA_DIR = Path("./data_memvid/eCert")
...
def benchmark(
    ...
    queries_file: Annotated[Path, typer.Option(...)] = DEFAULT_QUERIES_FILE,
    num_queries_to_run: Annotated[int, typer.Option(...)] = 0,
    repetitions: Annotated[int, typer.Option(...)] = 3,
    top_k: Annotated[int, typer.Option(...)] = 5,
    recompute_scores: Annotated[bool, typer.Option(help="Optionally recompute similarity scores manually.")] = False,
):
    """
    ...
    for i, query_text in enumerate(queries):
        ...
        for _ in range(repetitions):
            ...
            if recompute_scores:
                ...
                print("[DEBUG] Recomputing cosine similarity scores...")
                query_embedding = model.encode(query_text, convert_to_tensor=True)
                chunk_embeddings = model.encode(results, convert_to_tensor=True)
                scores = util.pytorch_cos_sim(query_embedding, chunk_embeddings)[0]
                ranked = sorted(zip(scores.tolist(), results), reverse=True)
                print("[RECOMPUTED SIMILARITY RANKING]")
                for j, (score, chunk) in enumerate(ranked):
                    preview = chunk[:90].replace("\n", " ")
